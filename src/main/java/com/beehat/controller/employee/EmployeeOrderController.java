...
public class EmployeeOrderController {
    ...
    Page<Invoice> listInvoice(@RequestParam(defaultValue = "0") int page,
        ...
        Page<Invoice> invoicePage = invoiceRepo.searchInvoices1(searchTerm,invoiceType,startDate,endDate,pageable);
    ...
    List<Invoice> listInvoiceCanceled() {
        List<Invoice> list = invoiceRepo.findByStatus((byte) 1);
        List<Invoice> filteredInvoices = list.stream()
                .filter(invoice -> invoice.getInvoiceStatus() == 1) // Lọc status = 1
